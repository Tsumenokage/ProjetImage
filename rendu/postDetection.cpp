...
void contoursTerrain(const Mat src, Mat &dst) {
  ...
  if(!bench)
	imshow( "Enveloppe convexes du terain", hullP );
  ...
  while(1)
	...
	break;
      ...
      if ( !tooMuchGrass(grass)) {
        ...
        if(!bench)
          imshow("Fusion des canaux",output);
        ...
        if(!bench)
          imshow("Image binaire des poteaux detectés",last);
      ...
      if(!bench)
        {
          imshow("Detection des poteaux",image);
          waitKey();
        }
    }
  clock_t end = clock();
